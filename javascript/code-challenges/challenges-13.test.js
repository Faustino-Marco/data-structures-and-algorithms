...
let runningLong = 0;
arr.forEach((str, idx) => {
  if (str.length > runningLong) {
    runningLong = str.length;
    index = idx;
  }
})
return index;
...
const standardizePhoneNumbers = (arr) => {
  ...
  return arr.map(num => {
    ...
    let midNums = num.substring(6,9);
    let lastNums = num.substring(10);
    return `${areaCode}${midNums}${lastNums}`;
  })
};
...
const onlyOddChars = (str) => {
  ...
  let answer = newArr.filter((element, idx) => idx % 2 === 1);
  return answer.join('');
};
